...
class IsPrimeFrame(wx.Frame):
    def __init__(self, superion):
        wx.Frame.__init__(self, parent=superion, title='Check Prime', size=(300,200))
        panel = wx.Panel(self)
        panel.SetBackgroundColour('sky blue')
        wx.StaticText(parent=panel, label='Input a integer:', pos=(10,30))
        self.inputN = wx.TextCtrl(parent=panel, pos=(120,30))
        self.result = wx.StaticText(parent=panel, label='', pos=(10,50))
        self.buttonCheck = wx.Button(parent=panel, label='Check', pos=(30,90))
        self.Bind(wx.EVT_BUTTON, self.OnButtonCheck, self.buttonCheck)
        self.buttonQuit = wx.Button(parent=panel, label='Quit', pos=(140,90))
