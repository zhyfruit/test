...
class wxGUI(wx.App):
    def OnInit(self):
        ...
        bmp = wx.Image("bgd.bmp",  wx.BITMAP_TYPE_ANY)
        sb = wx.StaticBitmap(self.panel, -1, wx.BitmapFromImage(bmp))
        ...
        self.toolbar.AddSimpleTool(9998,wx.Image('color.png',\
                                                 wx.BITMAP_TYPE_PNG).ConvertToBitmap(),\
                                   'Color', 'Click to Open Color Dialog')
        self.toolbar.Realize()
        wx.EVT_TOOL(self, 9999, self.OnOpen)
        wx.EVT_TOOL(self, 9998, self.OnColor)
    ...
    def OnColor(self, event):
        ...
        dlg.ShowModal()
        c = dlg.GetColourData()
        wx.MessageBox(str(c.Colour))
        self.panel.SetBackgroundColour(c.Colour)
        self.image = wx.StaticBitmap(self.panel, size=self.panel.GetSize())
        self.panel.Refresh()
