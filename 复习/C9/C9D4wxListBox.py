...
class ListBoxDemo(wx.Frame):
    def __init__(self, superion):
        wx.Frame.__init__(self, parent=superion, title='ListBox demo', size=(200,280))
        panel = wx.Panel(self)
        self.buttonQuit = wx.Button(parent=panel, label='Quit', pos=(40,180))
        self.Bind(wx.EVT_BUTTON, self.OnButtonQuit, self.buttonQuit)
        li = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday']
        self.listBox = wx.ListBox(panel, choices=li, pos=(30,30))
