...
def func6():
    f=[filename for filename in os.listdir(r'c:\\') if filename.endswith(('.bmp','.jpg','.txt'))]
    print f

def func7():
    example = 'School of Computer Science and Technology, Shanghai University'
    pattern = re.compile(r'\bS\w+\b') #以S开头的单词
    print pattern.findall(example)
    pattern = re.compile(r'\w+y\b')#以y结尾的单词
    print pattern.findall(example)
    pattern = re.compile(r'\b\w*o\w*\b')#查找所有含有字母o的单词
    print pattern.findall(example)
    pattern = re.compile(r'\b[a-zA-Z]{3}\b')#查找3个字母长的单词
    print pattern.findall(example)
    print pattern.match(example)#从字符串开头开始匹配，所以不成功，没有返回值
    print pattern.search(example)#在整个字符串中搜索，所以成功
...
yours is 010-12345678, his is 025-87654321.'''
    ...
    while True:
        ...
        for i in range(3):
            ...
            ' Start from:',matchResult.start(i),'End at:',\
